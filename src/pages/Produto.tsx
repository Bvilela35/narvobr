...
export default function Produto() {
  ...
  // Reset selections when handle changes
  useEffect(() => {
    setSelectedImage(0);
          ...
          <div className="grid md:grid-cols-[1.1fr_0.9fr] gap-10 md:gap-16">
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6 }} className="relative">
              <div className="aspect-square bg-card-elevated rounded-2xl overflow-hidden">
                {imgs[selectedImage] ? (
                  <img src={imgs[selectedImage].node.url} alt={imgs[selectedImage].node.altText || title} className="w-full h-full object-contain" />
                ) : (
              ...
              {totalImages > 1 && (
                <div className="flex items-center gap-4 mt-4">
                  <button onClick={prevImage} className="w-9 h-9 rounded-full border border-border flex items-center justify-center hover:border-foreground/40 transition-colors" aria-label="Imagem anterior">
                    <ChevronLeft className="h-4 w-4" />
                  </button>
                  <span className="text-sm tabular-nums opacity-60">{selectedImage + 1} / {totalImages}</span>
                  <button onClick={nextImage} className="w-9 h-9 rounded-full border border-border flex items-center justify-center hover:border-foreground/40 transition-colors" aria-label="Próxima imagem">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }} className="flex flex-col">
                    ...
                    <div key={option.name} className="mb-6">
                      ...
                      <div className="flex flex-wrap gap-3">
                        {variants.edges.map((v, i) => {
                          ...
                          return (
                            <button key={v.node.id} onClick={() => setSelectedVariantIdx(i)}
                              className={`px-5 py-3 rounded-xl border-2 text-sm transition-all min-w-[80px] text-center ${isSelected ? 'border-foreground' : 'border-border hover:border-foreground/30'} ${!v.node.availableForSale ? 'opacity-30 line-through cursor-not-allowed' : ''}`}
              ...
              <Button onClick={handleAddToCart} disabled={isCartLoading || !selectedVariant?.availableForSale} className="h-13 rounded-lg text-sm font-medium tracking-wide w-full">
                {isCartLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : "Adicionar ao carrinho"}
              </Button>

              {!selectedVariant?.availableForSale && <p className="text-sm opacity-40 mt-3">Esgotado</p>}
